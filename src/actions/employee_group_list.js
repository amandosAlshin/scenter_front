...
export const serverDispatch = ()=> ({
  type: SERVER_DISPATCH+'EMPLOYEE_GROUP_LIST',
})
export const dispatchSuccess = (data)=> ({
  type: DISPATCH_SUCCESS+'EMPLOYEE_GROUP_LIST',
  data: data,
})
export const dispatchError = (msg)=> ({
  type: DISPATCH_ERROR+'EMPLOYEE_GROUP_LIST',
  msg: msg
})
export const EmployeeGroupAction = (data)=>{
  return (dispatch,getState)=>{
    dispatch(serverDispatch())
        const AuthStr = 'Bearer '.concat(sessionStorage.nomad_auth);
      axios({
        method: 'post',
        url: SERVER+'api/employee/employee_group_list',
        headers: {
          'crossDomain': true,
