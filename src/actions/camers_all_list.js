...
export const serverDispatch = ()=> ({
  type: SERVER_DISPATCH+'CAMERA_ALL_LIST',
})
export const dispatchSuccess = (data)=> ({
  type: DISPATCH_SUCCESS+'CAMERA_ALL_LIST',
  data: data,
})
export const dispatchError = (msg)=> ({
  type: DISPATCH_ERROR+'CAMERA_ALL_LIST',
  msg: msg
})
export const CameraAllListAction = ()=>{
  return (dispatch,getState)=>{
    dispatch(serverDispatch())
      const AuthStr = 'Bearer '.concat(sessionStorage.nomad_auth);
      axios({
        method: 'post',
        url: SERVER+'api/camera/cameraalllist',
        headers: {
          'crossDomain': true,
